...
## 2026-05-11T08:30Z — End-of-session reframe: Layer 1 becomes the playable cartridge

**File:** docs/design/oneshot.md (new file)
**After:** Comprehensive Layer-1 design doc (~14 sections) consolidating every decision made in the closing turn of the session: temp Dungeon Digger asset import (player / tiles / doors / NPCs, no monsters), 32×32 tile + sprite sizing, 16×16 element / item icons + 32×32 portraits (in `assets/graphics/icons/` + `assets/graphics/portraits/`), Y-opens / B-closes menu rebind, navigable inventory + use-outside-battle for potions, the seven-stat schema (HP / MP / ATK / DEF / MATK / MDEF / SPD), FFX-style CTB scheduler with upcoming-turns strip, MP-as-placeholder note + future stamina / rhythm-game intent, per-enemy ability + basic attack split with Layer-1 effect definitions for `curse` / `firebolt` / `wave_fist` / `heal` / `shakas_light` / `spirit_blast` / `fire_punch`, the rounded-corner / white-border / black-fill window-frame style decision applied to overworld + battle + menus, the village-hub + 4–6-cell-dungeon shape, save / heal / shop tile interactables, battle balance targets, and the nine-pass implementation order.
**Why:** Frankie's end-of-session message reframed the project around a one-shot dungeon as Layer 1's deliverable, absorbing what was originally split across Layers 1 / 2 / 3. Capturing the full set of decisions in one document so the work has a single source of truth and so future contributors (including future-me) don't have to re-derive the design from the conversation transcript.
**Editor:** Frankie (Claude Opus 4.7)

**File:** docs/design/patterns-from-predecessors.md (new file)
**After:** Code-pattern reference (~15 sections) capturing every reusable idiom from the Dungeon Digger and Adventure projects so neither needs to be referenced again. Sections cover: sprite loading + scaling cache pattern, multi-variant sprite dict keyed by gameplay state, walk-cycle animation (target_pos lerp, already shipped in Nuitai), idle "look-around" animation, asset directory iteration with `_left` / `_right` suffix pairing, NPC + door sprite shapes, cardinal-only step normalisation, the stateless coords helper, the cell-based world model (already lifted into `core/world.py`), the char-grid → blits tile renderer (already shipped, ready for sprite swap in Pass 3), `color_with_alpha` utility, the tutorial-card overlay system as an optional Layer-1.5 pattern, and the per-tile state dict pattern as an alternative to char grids when cells outgrow them. Closes with an explicit "patterns we deliberately are NOT lifting" list (`MessageLog`, `DungeonLevel`, fog-of-war / LOS, dig / treasure, monster AI, smooth-motion AABB, leaderboard) so the choice not to port them is recorded.
**Why:** Frankie wants to be able to continue without looking at the predecessor repos again (other than asset files he's importing manually). This file is the answer to "what do we still need from those projects?" — every code idiom worth carrying lives here in idiomatic Nuitai shape.
**Editor:** Frankie (Claude Opus 4.7)

**File:** docs/ROADMAP.md
**Lines (at time of edit):** preamble + Layer 1 / 2 / 3 sections (significantly rewritten)
**Before:** Layer 1 was "vertical-slice text demo" (~20–30 minutes, no sprites, free portrait placeholders). Layer 2 was "content & polish on the same engine" (town hub, second dungeon, status ailments, equipment). Layer 3 was "Dragon Quest stage" (tile-based world map, sprite encounter view, animated overworld sprites, ship system).
**After:** New preamble notes the 2026-05-11 reframe and explains why the layer ramp compressed (the engine has been de-risked by Pass 1 / Pass 2 of the overworld track, so the staged "text → text+portraits → tile-and-sprite" ramp lost most of its value). Layer 1 is now "the one-shot (the playable cartridge)" — full game with overworld + dungeon + boss + NPCs + shop + save/heal points + CTB battles + seven-stat system, ~30 minutes, ships as a complete short JRPG. Layer 2 contracts to "bigger world, deeper systems" (second region, Maika, status ailments, equipment, stance / augment system, MP-replacement). Layer 3 reframes from "Dragon Quest stage" to "the world map and the maelstrom" (since the tile-and-sprite engine ships in Layer 1, this layer is now about sailing and the maelstrom-zone speed system).
**Why:** Captures the pivot the user articulated at session close. The original ramp was right when the engine was untested; with the engine proven, holding back content behind layer gates would slow ship rather than de-risk it. Layer 1 absorbs the foundational work so later layers can focus on world / story / art / music.
**Editor:** Frankie (Claude Opus 4.7)

**File:** docs/TODO.md
**Lines (at time of edit):** Layer 0 heading (marked "shipped"), Layer 1 section (entirely rewritten as a nine-pass plan), Layer 0.5 section (rewritten to note the engine-seam items absorbed into Layer 1), Layer 3 — Overworld section (marked "shipped"), historical Pass-1 / Pass-2 items annotated with their new Layer-1 pass homes
**Before:** TODO had a flat Layer-1 list of writing / content / placeholder-art tasks oriented around the old text-only Layer-1 spec; an open Layer-0.5 deferral list with three items; a Layer-3 (Overworld) parallel-track section with Pass-1 done and Pass-2 partially done; a cross-cutting always-on section.
**After:** Layer 1 is a structured nine-pass plan (Pass 3 asset bones → Pass 4 stats + CTB → Pass 5 icons + portraits → Pass 6 inventory navigation → Pass 7 dungeon content → Pass 8 balance + writing → Pass 9 Layer-0.5 fold-in) with a clear Definition of Done. Layer 0.5 calls out that scene-stack-in-save and `Combatant.statuses` have been folded into Layer-1 Pass 9 (the latter sooner if Pass 4's `curse` lands first). Layer 3 — Overworld is marked shipped historical with each still-open Pass-2 item annotated with its new Layer-1 pass home (sprite sourcing → Pass 3, NPC / sign / warp wiring → Pass 7, mid-transition save → Pass 9, footstep audio → Pass-3 stretch or defer). Open design questions resolved (menu key on Tab keyboard + Y controller; cell JSON migration likely yes during Pass 7; panel chrome decision committed in oneshot.md §10).
